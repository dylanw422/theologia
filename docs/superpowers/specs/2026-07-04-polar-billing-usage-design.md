...
2. **Enforcement:**
   - **Free:** hard limit of **20 queries per calendar month** (UTC; resets the 1st at 00:00 UTC). Counted per user message sent, incremented transactionally in the send mutations — no dollar budget and no race window.
   - **Paid:** hard cap at 100% of the weekly dollar budget. Over-cap sends fail with a typed error; the UI tells paid users to upgrade or wait for the reset. No unlimited overage, no Haiku downgrade for over-cap users.
...
| planId       | productKey (Polar) | label       | model             | limit                    |
| ------------ | ------------------ | ----------- | ----------------- | ------------------------ |
| `free`       | — (no sub)         | Free        | claude-haiku-4-5  | 20 queries/calendar month |
| `scholar`    | `scholar`          | Scholar     | claude-sonnet-5   | $1.375/week              |
| `ministry`   | `ministry`         | Ministry    | claude-sonnet-5   | $2.825/week              |
| `churchTeam` | `churchTeam`       | Church Team | claude-sonnet-5   | $7.20/week               |

- Paid budgets = monthly API budget from PRICING.md ÷ 4 ($5.50, $11.30, $28.80).
- **Free is query-counted, not dollar-metered** — exactly 20 queries per calendar month, no more, no less. Free runs Haiku, so worst case is ~$0.30/user/month. PRICING.md gets a follow-up edit reflecting free-on-Haiku and the query-count enforcement.
...
usageMonths: {
  userId: string,
  monthStart: number,         // ms epoch of the 1st, 00:00 UTC
  queries: number,            // free-tier query counter
}  index by_user_month on [userId, monthStart]
```

Micro-USD integers avoid float drift on repeated accumulation. `usageWeeks` still accumulates for every user (including free) for cost observability; it just isn't what free enforcement reads.
...
- `getUsage` query (auth required): returns a discriminated shape —
  free: `{ planId: "free", planLabel, kind: "queries", used, limit: 20, resetsAt }` (next month start, UTC);
  paid: `{ planId, planLabel, kind: "budget", usedUsd, weeklyBudgetUsd, resetsAt }` (next Monday, UTC).
  Plan resolved via `getCurrentSubscription`.

### Enforcement (`chat.ts`)

- `createConversation` and `sendMessage`: resolve plan.
  - Free: read the `(userId, monthStart)` row; if `queries >= 20`, throw `ConvexError({ code: "USAGE_LIMIT", planId: "free" })`; otherwise increment the counter in the same mutation (transactional — no race).
  - Paid: read current week usage; if `usedUsd >= weeklyBudgetUsd`, throw `ConvexError({ code: "USAGE_LIMIT", planId })`.
  - On a throw, the prompt is not saved and no reply is scheduled.
...
- Race window (usage recorded after generation completes) means a **paid** user can slightly exceed the cap on their last message; acceptable — the next send is blocked. Free has no race: the counter increments in the send mutation itself.

## Frontend (apps/web)

### Chat usage meter

- `chat-usage-meter.tsx` consumes `api.usage.getUsage` via `useQuery`; `mock-usage.ts` deleted. Paid: plan label, percent of weekly budget used, countdown to the UTC Monday reset. Free: "X of 20 queries" and countdown to the monthly reset.
...
- Unit: cost computation per model (incl. cache token rates, micro-USD rounding), week-start/reset math around UTC Monday boundaries, month-start/reset math around UTC month boundaries.
